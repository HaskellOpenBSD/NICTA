...
(|||) ::
  ...
  -> Parser a
(|||) pA pA' =
  P $ \a -> if isErrorResult (parse pA a) then (parse pA' a) else (parse pA a)
...
satisfy ::
  ...
  -> Parser Char
satisfy p =
  (\r -> if p r then pure r else unexpectedCharParser r) `bindParser` character
...
natural ::
  Parser Int
natural =
  (read <$> list1 digit) `flbindParser` \as -> case as of
    Empty -> failed
    Full result -> valueParser result
...
sequenceParser (pA :. pAs) =
  pA `flbindParser` (\a ->
  sequenceParser pAs `flbindParser` (\as ->
  valueParser (a :. as)))
...
surnameParser =
  upper `flbindParser` (\a ->
  thisMany 5 lower `flbindParser` (\as ->
  list lower `flbindParser` (\beyond5 ->
  valueParser ((a :. as) ++ beyond5))))
